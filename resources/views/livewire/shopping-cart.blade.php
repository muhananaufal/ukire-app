    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
        @if ($cartItems->isEmpty())
            <div class="text-center" data-aos="fade-up">
                ...
                <p class="mt-4 text-base text-gray-500">Sepertinya Anda belum menemukan karya yang tepat. Mari kita cari
                    bersama.</p>
        ...
        @else
            <div class="text-center" data-aos="fade-up">
                ...
                <p class="mt-2 max-w-2xl mx-auto text-lg text-gray-600">Pilih item yang ingin Anda checkout, atau
                    perbarui pesanan Anda.</p>
            </div>
                ...
                <div class="lg:col-span-2 space-y-6" data-aos="fade-right">
                    <div class="flex justify-between items-center pb-4 border-b border-gray-200">
                        <div class="flex items-center space-x-3">
                            <input type="checkbox" @change="$wire.toggleAll($event.target.checked)"
                                class="h-4 w-4 rounded border-gray-300 text-amber-600 focus:ring-amber-500"
                                :checked="Object.values(@js($selectedItems)).every(value => value === true) && Object
                                    .values(@js($selectedItems)).length > 0">
                    ...
                    @foreach ($cartItems as $item)
                        <div class="flex items-start bg-white border border-gray-200 rounded-lg p-6 shadow-sm"
                            ...
                            <input type="checkbox" wire:model.live="selectedItems.{{ $item->id }}"
                            ...
                            <div class="ml-6 flex-grow">
                                <a href="{{ route('catalog.show', $item->attributes->slug) }}">
                                    <h3 class="text-lg font-bold text-gray-900 hover:text-amber-600">{{ $item->name }}
                                    </h3>
                                </a>
                        ...
                        <div class="mt-8">
                            @if (collect($selectedItems)->filter()->count() > 0)
                                <button wire:click="proceedToCheckout" wire:loading.attr="disabled"
                                    class="w-full block text-center bg-gray-900 text-white py-3 px-4 rounded-md hover:bg-gray-700 transition-colors font-semibold disabled:opacity-50 disabled:cursor-not-allowed">
                                    Lanjutkan ke Checkout ({{ collect($selectedItems)->filter()->count() }} item)
                                </button>
                            @else
                                <button
                                    class="w-full block text-center bg-gray-300 text-gray-500 py-3 px-4 rounded-md font-semibold cursor-not-allowed">
                                    Pilih Item Dahulu
                                </button>
                            @endif
                        </div>
