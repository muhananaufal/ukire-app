<div>
    <section class="relative bg-gray-800 h-64 flex items-center justify-center text-center">
        <div class="absolute inset-0">
            <img src="https://images.unsplash.com/photo-1524758631624-e2822e304c36?q=80&w=2070"
                alt="Latar belakang tekstur kayu atau interior yang elegan" class="w-full h-full object-cover opacity-20">
        </div>
            ...
            <aside class="lg:sticky lg:top-28 space-y-8" data-aos="fade-right">
                ...
                <div>
                    ...
                    <select wire:model.live="sort" id="sort"
                        class="mt-2 block w-full rounded-md border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500 sm:text-sm">
            ...
            <main class="lg:col-span-3">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between pb-4 border-b border-gray-200">
                    <div class="flex items-center space-x-2 flex-wrap gap-y-2">
                        @if ($search || $category)
                            ...
                            @if ($search)
                                <span
                                    class="inline-flex items-center py-1 px-3 rounded-full text-xs font-medium bg-gray-100 text-gray-800">
                                    Cari: {{ $search }}
                                    <button wire:click="$set('search', '')"
                                        class="ml-2 text-gray-500 hover:text-gray-800 font-bold">&times;</button>
                                </span>
                            @endif
                            @if ($category)
                                <span
                    ...
                    <div class="flex items-center space-x-1 mt-4 sm:mt-0">
                        <button wire:click="$set('viewType', 'grid')"
                            class="{{ $viewType == 'grid' ? 'bg-gray-800 text-white' : 'bg-white text-gray-500 hover:bg-gray-100' }} p-2 rounded-md border border-gray-300 transition-colors">
                        ...
                        <button wire:click="$set('viewType', 'list')"
                            class="{{ $viewType == 'list' ? 'bg-gray-800 text-white' : 'bg-white text-gray-500 hover:bg-gray-100' }} p-2 rounded-md border border-gray-300 transition-colors">
                            <svg class="h-5 w-5" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd"
                ...
                <div class="mt-8" wire:loading.class.delay="opacity-50 transition-opacity">
                            ...
                            @forelse ($products as $product)
                                <div data-aos="fade-up" data-aos-delay="{{ ($loop->index % 3) * 50 }}">
                                    ...
                                    <button wire:click="quickView({{ $product->id }})"
                                        class="w-full mt-2 text-sm font-semibold text-center text-gray-600 bg-gray-100 hover:bg-gray-200 py-2 rounded-md transition-colors">Lihat
                                        Cepat</button>
                                </div>
                            @empty
                                <div class="col-span-full text-center py-16">
                                    ...
                                    <a href="#" wire:click.prevent="clearFilters"
                                        class="mt-6 inline-block bg-gray-800 text-white py-2 px-6 rounded-md hover:bg-gray-700">
                                        Hapus Semua Filter
                                    </a>
                                </div>
                            @endforelse
                    ...
                    @else
                        <div class="space-y-8">
                            @forelse ($products as $product)
                                <div class="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-6 p-4 border rounded-lg"
                                    data-aos="fade-up">
                                    <a href="{{ route('catalog.show', $product) }}"
                                        class="block sm:w-40 sm:flex-shrink-0">
                                    ...
                                    <div class="flex-grow">
                                        ...
                                        <p class="mt-2 text-sm text-gray-600 leading-relaxed">{!! Str::limit(strip_tags($product->description), 200) !!}
                                        </p>
                                    </div>
                                    <div
                                        class="sm:w-48 sm:flex-shrink-0 sm:text-right flex flex-col items-start sm:items-end">
                                        <p class="text-xl font-bold text-gray-800">Rp
                                            {{ number_format($product->price / 100, 0, ',', '.') }}</p>
                                        <button wire:click="quickView({{ $product->id }})"
                                            class="w-full sm:w-auto mt-4 text-sm font-semibold text-center text-gray-600 bg-gray-100 hover:bg-gray-200 py-2 px-4 rounded-md transition-colors">Lihat
                                            Cepat</button>
                                    </div>
                                </div>
                            @empty
    ...
    @if ($quickViewProduct)
        <div class="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4" x-data="{ open: true }"
            x-show="open" @keydown.escape.window="open = false; setTimeout(() => $wire.closeQuickView(), 300)"
            ...
            <div class="bg-white rounded-lg max-w-4xl w-full grid grid-cols-1 md:grid-cols-2 overflow-hidden"
                @click.away="open = false; setTimeout(() => $wire.closeQuickView(), 300)" x-show="open"
                ...
                <div class="h-64 md:h-full bg-gray-100">
                    <img src="{{ asset('storage/' . $quickViewProduct->images->first()?->image_path) }}"
                ...
                <div class="p-8 flex flex-col">
                    ...
                    <div class="mt-8 border-t pt-6">
                        <p class="text-2xl font-bold text-gray-900">Rp
                            {{ number_format($quickViewProduct->price / 100, 0, ',', '.') }}</p>
                        <a href="{{ route('catalog.show', $quickViewProduct) }}"
                            class="mt-2 block text-sm text-amber-600 hover:text-amber-500 font-medium">Lihat detail
                            lengkap &rarr;</a>
                        <form action="{{ route('cart.store') }}" method="POST" class="mt-4">
                            @csrf
                            <input type="hidden" name="product_id" value="{{ $quickViewProduct->id }}">
                            <button type="submit"
                                class="w-full bg-gray-800 text-white py-3 px-6 rounded-md text-base font-semibold hover:bg-gray-700 transition-colors">
                                Pre-Order Sekarang
                            </button>
                        </form>
                    </div>
                </div>
                <button @click="open = false; setTimeout(() => $wire.closeQuickView(), 300)"
