<x-app-layout>
    <div class="bg-gray-50">
        <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
            @if (\Cart::isEmpty())
                {{-- WOW: Tampilan Keranjang Kosong yang Lebih Baik --}}
            ...
            @else
                ...
                <div class="text-center" data-aos="fade-up">
                    ...
                    <p class="mt-2 max-w-2xl mx-auto text-lg text-gray-600">Harap periksa kembali pesanan Anda sebelum melanjutkan.</p>
                </div>
                ...
                <div class="mt-12 grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-16 items-start">
                            ...
                            <div class="flex items-start bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
                                ...
                                <div class="ml-6 flex-grow">
                                    <a href="{{ route('catalog.show', $item->attributes->slug) }}" class="block">
                                        <h3 class="text-lg font-bold text-gray-900 hover:text-amber-600">{{ $item->name }}</h3>
                                    </a>
                                    ...
                                    <div class="mt-4 flex items-center">
                                        <form action="{{ route('cart.update', $item->id) }}" method="POST" class="flex items-center">
                                            ...
                                            <input type="number" name="quantity" value="{{ $item->quantity }}" min="1" id="quantity-{{ $item->id }}" class="w-16 text-center rounded-md border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500 sm:text-sm">
                                            <button type="submit" class="ml-3 text-xs font-semibold text-amber-600 hover:text-amber-500">Update</button>
                                        </form>
                                    </div>
                                </div>
                                <div class="ml-4 flex-shrink-0">
                                    <form action="{{ route('cart.destroy', $item->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-gray-400 hover:text-red-500 transition-colors" aria-label="Hapus item">
                    ...
                    <aside class="lg:col-span-1 lg:sticky lg:top-28" data-aos="fade-left">
                        <div class="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
                            ...
                            @auth
                                <a href="{{ route('checkout.index') }}" class="mt-8 w-full block text-center bg-gray-900 text-white py-3 px-4 rounded-md hover:bg-gray-700 transition-colors font-semibold">
                                    Lanjutkan ke Checkout
                                </a>
                            @else
                                <a href="{{ route('login', ['redirect' => 'cart.index']) }}" class="mt-8 w-full block text-center bg-gray-900 text-white py-3 px-4 rounded-md hover:bg-gray-700 transition-colors font-semibold">
                                    Masuk untuk Melanjutkan
                                </a>
                            @endauth

                            {{-- WOW: Sinyal Kepercayaan (Trust Signals) --}}
